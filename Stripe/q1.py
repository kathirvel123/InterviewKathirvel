...
They will possibly report this question like other, so stay vigilent. And post it to somewhere else if necessary.
'''

def part1(log : String , closing_time : Int):
    cur = 'N'
    log = log.split()
    p = 0
...
def part2(log:String):
    ans = float('inf')
    for i in range(len(log.split())):
        ans = min(ans ,part1(log,i))
    return ans
